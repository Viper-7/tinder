def getRDocMethod(classname,methodname="")
			...
			if mnames.include?('<br')
				mnames.scan(/(.+?)<br[ \/]*>/im) {|mname|
					...
					if methodname == ""
						...
						outstr += outs if outs.length > 1 and !outstr.include?(outs) and !outarr.each{|x| if x.match(/#{outs}/i); return true; else; return false; end}
	...
	if outarr.length == 0 and !(outstr.length > 1)
		outarr.push outstr
		outarr.each {|x|
			puts x.chomp
		}
