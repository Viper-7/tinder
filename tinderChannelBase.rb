...
class TinderChannel
    ...
    def poll
		...
		@tinderBot.clearPing if @ping and (@uptime + 470) % 480 == 0
    ...
    def runCommand(command, args, nick, host, commandtypes)
    	sendChannel 'Searching, This may take a few moments' if command == 'nzbsearch' and @channel != 'www'
    	response = ""
    	hit = false
    	commandtypes.each{|z|
    		folders = ["scripts/#{z}/builtin","scripts/#{z}/user"]
	...
	case command.chomp
		when /^php$/
			if args == "" and @channel == 'www'
				...
				args.gsub!(/(?:114, 110)|rm|-rf|mail|Socket|fsockopen|exit/i,'BLOCKED COMMAND')
				args = "<?php\n" + args.gsub(/&\$p/,'+') + "\n?>"
		...
		when /^ruby$/
			if args == "" and @channel == 'www'
				...
				args.gsub!(/(?:114, 110)|rm|-rf|mail|socket/i,'BLOCKED COMMAND')
		...
		when /^perl$/
			if args == "" and @channel == 'www'
				...
				args.gsub!(/(?:114, 110)|-rf|rm|mail/i,'BLOCKED COMMAND')
				args = args.gsub(/\[\\n\]/, "\n")
	...
	if channel != 'www'
		...
		response = response.gsub(/<\/?(?:B|STRONG)>/i,'').gsub(/<[\/]?TITLE.*?>/i,'').gsub(/<A.+?HREF\s?=\s?(?:'|")(http.+?)(?:'|").*?>(.*?)<\/A>/i,'\2 [ \1 ] ').gsub(/<(?:style|script)>.+?<\/(?:style|script)>/i,'').gsub(/\n/m,'').gsub(/<BR[\/]?>/,"\n").gsub(/<[^>]*>/,'').gsub(/&lt;/,'<').gsub(/&nbsp;/,'_').gsub(/&gt;/,'>').chomp
		@tinderBot.status "Output  : " + response if @tinderBot
	else
		response.gsub!(/\n/,'<BR/>') if !response.match(/<[^>]*>/) and response != ''
		response.gsub!(/[\002]([^\002]+)(?:[\002]|$)/,'<STRONG>\1</STRONG>') if response != ''
	end
	return response
    end
